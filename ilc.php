				...
				<div class="panel-heading">
				  ...
				  <form id="invitedForm" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="post"  >
				  	<input class="btn btn-md btn-primary" type="submit" value="Save" name="ilc_save" />
							<select name="il" style="width: 220px" onchange="showUser(this.value,this.name)">
								<option>--Title--</option>
								<?php 
									include('DBConnect.php');
					   ...
					   <div id="invitedFields">
						   ...
						   <label>Title Conference / Seminar etc.</label>
							...
							<label>Date (s) of the Event</label>
							 <input class="form-control" type="text" onkeypress="return onlyAmpersand(event)" name="ILC_DOE" required="required" placeholder="yy-mm-day"/> 
	 ...
	 <script>
		...
		xmlhttp.onreadystatechange=function()
		  ...
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{  
				if(name == 'il'){
					document.getElementById("invitedFields").innerHTML=xmlhttp.responseText;
				}
